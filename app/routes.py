"""HTTP routes for notifications-service.

Generic CRUD over the `notifications` table. Every mutating endpoint publishes a
...
def build_blueprint(svc) -> Blueprint:
    ...
    def send():
        payload = request.get_json(silent=True) or {}
        row = db.query_one(f"INSERT INTO {TABLE} (data) VALUES (%s) RETURNING *", (Json(payload),))
        bus.publish("notification.requested", key=str(row["id"]), value=payload)
        emit_audit(bus, action="notification.send", actor=_actor(), target=f"notification:{row['id']}")
